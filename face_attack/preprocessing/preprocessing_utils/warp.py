## usage example
# srim = cv2.imread('img1')
# ref = cv2.imread('refimg')
# height, width, depth = imgref.shape
# warped_image = warpe_image(srim, lm1, lmref, (height, width))
...
def warp_image(src_img, src_points, dest_points, dest_shape, dtype=np.uint8):
    ...
    src_pts, dest_pts = np.copy(src_points), np.copy(dest_points)
    # add boundary points to keep background
    bound = np.array([[2, dest_shape[0] - 3],
                      [dest_shape[1] - 3, dest_shape[0] - 3],
                      [dest_shape[1] - 3, 2],
                      [2, 2]])
    src_pts = np.vstack((src_pts, bound))
    dest_pts = np.vstack((dest_pts, bound))
    ...
    delaunay = spatial.Delaunay(dest_pts)
    tri_affines = np.asarray(list(triangular_affine_matrices(delaunay.simplices, src_pts, dest_pts)))

    process_warp(src_img, result_img, tri_affines, dest_pts, delaunay)
